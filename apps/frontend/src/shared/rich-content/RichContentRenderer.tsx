...
const imageUrlByAssetId = (images: RichContentImageAsset[]) => {
...
const renderMarkedText = (text: string, marks: RichContentMark[] | undefined, key: string, hideYouTubeLinks: boolean): React.ReactNode => {
...
const renderChildren = (
  content: RichContentNode[] | undefined,
  keyPrefix: string,
  images: Map<string, RichContentImageAsset>,
  hideYouTubeLinks: boolean,
): React.ReactNode[] => {
  return content?.map((node, index) => renderNode(node, `${keyPrefix}-${index}`, images, hideYouTubeLinks)).filter(Boolean) ?? [];
};

const renderNode = (node: RichContentNode, key: string, images: Map<string, RichContentImageAsset>, hideYouTubeLinks: boolean): React.ReactNode => {
  if (node.type === 'text') {
...
export function RichContentRenderer({ content, images = [], hideYouTubeLinks = false }: RichContentRendererProps) {
  ...
  return (
    <article className="space-y-4">
      {(parsed.document.content ?? []).map((node, index) => renderNode(node, `rich-content-node-${index}`, imageMap, hideYouTubeLinks))}
      <YouTubeEmbeds videoIds={videoIds} />
    </article>
  );
}
