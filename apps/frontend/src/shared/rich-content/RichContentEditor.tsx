...
import {
  createRichContentDocumentFromText,
  emptyRichContentDocument,
  hydrateRichContentImages,
  parseRichContent,
  serializeRichContentDocument,
  type RichContentDocument,
  type RichContentImageAsset,
} from './utils';
...
type RichContentEditorProps = {
  ...
  onImageUpload?: (file: File) => Promise<RichContentImageAsset>;
...
const RichImageNode = Node.create({
  name: 'image',
  group: 'block',
  atom: true,
  draggable: true,
...
const editorContentFromValue = (value: string, images: RichContentImageAsset[] = []): RichContentDocument => {
  const parsed = parseRichContent(value);
  const document = parsed.kind === 'document' ? parsed.document : createRichContentDocumentFromText(parsed.text);
  return hydrateRichContentImages(document, images);
};
...
export function RichContentEditor({
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: currentEditor }) => {
      ...
      onChange(currentEditor.isEmpty ? '' : serializeRichContentDocument(json));
    },
  });
  ...
  useEffect(() => {
    ...
    const current = serializeRichContentDocument(editor.getJSON() as RichContentDocument);
    const next = serializeRichContentDocument(editorContentFromValue(value, images));
    if (current !== next) {
      editor.commands.setContent(value ? editorContentFromValue(value, images) : emptyRichContentDocument, { emitUpdate: false });
    }
  }, [editor, images, value]);
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 rounded-t-lg border border-slate-200 bg-slate-50 p-2">
        ...
        {onImageUpload && (
          <ToolbarButton label="Upload image" disabled={imageUploadDisabled} onClick={() => fileInputRef.current?.click()}>
