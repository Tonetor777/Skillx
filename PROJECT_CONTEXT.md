...
- Media uploads now support private MinIO/S3-compatible storage when S3 environment variables are configured, with internal upload endpoints separated from optional public signed URL hosts for profile photos, program thumbnails, and lesson images.
...
- Lesson reading views now include previous/next footer navigation that moves across lessons and same-week modules without crossing week boundaries, returning readers to the start of each selected lesson.
...
- Backend health checks now support both `GET` and `HEAD` requests at `/api/health/` for production load balancers and uptime monitors.
